...
double memory_limit = 3; // soft limit, in gigabytes, used to estimate optimal array sizes
...
int merge_files(int num) {
	...
	fwrite(&old, sizeof(CREC), 1, fout);
	fprintf(stderr,
			"\033[0GMerging cooccurrence files: processed %lld lines.\n",
			++counter);
	for (i = 0; i < num; i++) {
		sprintf(filename, "%s_%04d.bin", file_head, i);
		remove(filename);
...
int get_cooccurrence() {
	...
	bigram_table = (double *) calloc(lookup[a - 1], sizeof(double));
	...
	while (1) {
		...
		for (k = j - 1; k >= ((j > window_size) ? j - window_size : 0); k--) { // Iterate over all words to the left of target word, but not past beginning of line
			...
			if (w1 < max_product / w2) { // Product is small enough to store in a full array
				bigram_table[lookup[w1 - 1] + w2 - 2] += 1.0 / ((double) (j - k))*((double) (j - k)); // Weight by inverse of distance between words
				if (symmetric > 0)
					bigram_table[lookup[w2 - 1] + w1 - 2] += 1.0 / ((double) (j - k))*((double) (j - k)); // If symmetric context is used, exchange roles of w2 and w1 (ie look at right context too)
				...
				cr[ind].word1 = w1;
				cr[ind].word2 = w2;
				cr[ind].val = 1.0 / ((double) (j - k))*((double) (j - k));
	...
	if (verbose > 1)
		fprintf(stderr, "Writing cooccurrences to disk");
