...
int get_counts() {
	...
	while (fscanf(fid, format, str) != EOF) { // Insert all tokens into hashtable
		...
		if (((++i) % 100000) == 0)
			if (verbose > 1)
				fprintf(stderr, "\033[11G%lld tokens.", i);
	}
	if (verbose > 1)
	...
	if (verbose > 1)
		fprintf(stderr, "Counted %lld unique words.\n", j);
...
int main(int argc, char **argv) {
	...
	if ((i = find_arg((char *) "-max-vocab", argc, argv)) > 0)
		max_vocab = atoll(argv[i + 1]);
	if ((i = find_arg((char *) "-min-count", argc, argv)) > 0)
		min_count = atoll(argv[i + 1]);
	return get_counts();
}
