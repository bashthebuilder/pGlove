...
int shuffle_merge(int num) {
	...
	while (1) { //Loop until EOF in all files
		...
		if (verbose > 0)
			fprintf(stderr, "\033[31G%ld lines.", l);
	}
...
int main(int argc, char **argv) {
	...
	if ((i = find_arg((char *) "-temp-file", argc, argv)) > 0)
	...
	array_size = (long long) (0.95 * (real) memory_limit * 1073741824
			/ (sizeof(CREC)));
	if ((i = find_arg((char *) "-array-size", argc, argv)) > 0)
